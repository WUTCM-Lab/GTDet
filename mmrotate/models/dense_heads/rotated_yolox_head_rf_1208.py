...
class RotatedYOLOXHead_refine_1208(YOLOXHead):
    ...
    def init_weights(self):
        super(RotatedYOLOXHead_refine_1208, self).init_weights()
    ...
    def forward_single(self, x, cls_convs, reg_convs, conv_cls, conv_cls_rf, conv_reg,
        ...
        cls_score = conv_cls_rf(cls_feat)
        # 构造priors
        ...
        # print(anchor.shape)
        # print(rbox.shape)
        cls_feat = cls_convs_rf(cls_feat)
        reg_feat = reg_convs_rf(reg_feat)
        
        cls_score_rf = conv_cls(cls_feat)
        bbox_pred_rf = conv_reg_rf(reg_feat) + bbox_pred
        angle_pred_rf = conv_ang_rf(reg_feat) + angle_pred
