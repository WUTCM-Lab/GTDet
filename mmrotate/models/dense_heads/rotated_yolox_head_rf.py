...
class RotatedYOLOXHead_refine(YOLOXHead):
    ...
    def _init_layers(self):
        ...
        for _ in self.strides:
            ...
            conv_cls, conv_reg, conv_obj, conv_ang, conv_reg_rf, conv_ang_rf= self._build_predictor()
    ...
    def _build_predictor(self):
        ...
        return conv_cls, conv_reg, conv_obj, conv_ang, conv_reg_rf, conv_ang_rf
    

    def init_weights(self):
        super(RotatedYOLOXHead_refine, self).init_weights()
    ...
    def forward_single(self, x, cls_convs, reg_convs, conv_cls, conv_reg,
                       conv_obj, conv_ang, conv_reg_rf, conv_ang_rf, align_conv, stride, idx):
        ...
        rbox = rbox.detach()
        reg_feat = align_conv(reg_feat, rbox, stride)
        cls_feat = align_conv(cls_feat, rbox, stride)
        # print(anchor.shape)
        # print(rbox.shape)
        cls_score = conv_cls(cls_feat)
        bbox_pred_rf = conv_reg_rf(reg_feat) + bbox_pred.detach()
        angle_pred_rf = conv_ang_rf(reg_feat) + angle_pred.detach()
        objectness = conv_obj(reg_feat)
        return cls_score, bbox_pred_rf, angle_pred_rf, objectness, rbox
    ...
    def forward(self, feats):
        ...
        return multi_apply(
            ...
            self.multi_level_conv_ang_rf, self.multi_level_align_conv,
            self.strides, list(range(len(feats))) )

    @force_fp32(
        apply_to=('cls_scores', 'bbox_preds', 'angle_preds', 'objectnesses', 'rboxes'))
    def get_bboxes(self,
                   cls_scores,
                   bbox_preds,
                   angle_preds,
    ...
    def loss(self,
        ...
        # flatten_rbboxes = [ rbox for rbox in rboxes]
        
        flatten_cls_preds = torch.cat(flatten_cls_preds, dim=1)
        flatten_bbox_preds = torch.cat(flatten_bbox_preds, dim=1)
        flatten_angle_preds = torch.cat(flatten_angle_preds, dim=1)
        flatten_objectness = torch.cat(flatten_objectness, dim=1)
        # flatten_rbboxes = torch.cat(flatten_rbboxes, dim=0)
        flatten_priors = torch.cat(mlvl_priors)
        ...
        if self.separate_angle:
            ...
            loss_bbox_rf = self.loss_bbox_rf(
                flatten_rbboxes_rf.view(-1, 5)[pos_masks],
                bbox_targets) / num_total_samples
            # loss_bbox = self.loss_bbox(
            #     flatten_rbboxes.view(-1, 5)[pos_masks],
            #     bbox_targets) / num_total_samples
            # loss_bbox = self.loss_bbox(
            #     flatten_rbboxes[pos_masks],
            #     bbox_targets) / num_total_samples
        ...
        # loss_dict = dict(
        #     loss_cls=loss_cls, loss_bbox=loss_bbox, loss_obj=loss_obj, loss_bbox_rf=loss_bbox_rf)

        loss_dict = dict(
            loss_cls=loss_cls, loss_obj=loss_obj, loss_bbox_rf=loss_bbox_rf)
