...
class RotatedYOLOXHead_odm(YOLOXHead):
    ...
    def _init_layers(self):
        ...
        # or_conv or_pooling
        self.multi_level_or_conv = nn.ModuleList()
        self.multi_level_or_pooling = nn.ModuleList()
        for _ in self.strides:
            ...
            or_conv, or_pooling = self._build_or_convs()
            self.multi_level_or_conv.append(or_conv)
            self.multi_level_or_pooling.append(or_pooling)
    ...
    def _build_predictor(self):
        ...
        conv_cls, conv_reg, conv_obj = super(RotatedYOLOXHead_odm,
                                             self)._build_predictor()
        conv_ang = nn.Conv2d(self.feat_channels, self.angle_len, 1)
        return conv_cls, conv_reg, conv_obj, conv_ang

    def _build_or_convs(self):
        or_conv = ORConv2d(self.feat_channels, int(
                self.feat_channels ), kernel_size=3, padding=1, arf_config=(1, 8))
        # self.or_conv = ORConv2d(self.feat_channels, int(
        #         self.feat_channels / 8), kernel_size=3, padding=1, arf_config=(1, 8))
        or_pool = RotationInvariantPooling(self.feat_channels, 8)
        return or_conv, or_pool

    def init_weights(self):
        super(RotatedYOLOXHead_odm, self).init_weights()
    ...
    def forward_single(self, x, cls_convs, reg_convs, conv_cls, conv_reg,
                       conv_obj, conv_ang, or_conv, or_pooling):
    ...
    def forward(self, feats):
        ...
        return multi_apply(
            ...
            self.multi_level_conv_ang, self.multi_level_or_conv,
            self.multi_level_or_pooling)
