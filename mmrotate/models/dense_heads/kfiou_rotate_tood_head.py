...
class KFIoURotatedTOODHead(RotatedATSSHead):
    ...
    def __init__(self,
                 initial_loss_cls=dict(
                                ...
                                loss_weight=1.0),
                 **kwargs):
        super(KFIoURotatedTOODHead, self).__init__(**kwargs)
        self.epoch = 0
        if self.train_cfg:
    ...
    def _init_layers(self):
        ...
        self.reg_decomp = TaskDecomposition(self.feat_channels,
                                            ...
                                            self.stacked_convs * 8,
                                            self.conv_cfg, self.norm_cfg)
        # self.gcBlock = ContextBlock2d(self.feat_channels * self.stacked_convs,
        #                               self.feat_channels,
        #                               "att",
        #                               ["channel_add"])
        # ContextBlock2d(16,16,"att",["channel_add"])
    ...
    def loss_single(self, cls_score, bbox_pred, anchors, labels, label_weights,
        ...
        if len(pos_inds) > 0:
            ...
            pos_bbox_pred_decode = self.bbox_coder.decode(pos_anchors, pos_bbox_pred)
            pos_bbox_targets_decode = self.bbox_coder.decode(pos_anchors, pos_bbox_targets)
            ...
            # regression loss
            # print(bbox_weights)
            # pos_bbox_weight = self.centerness_target(pos_bbox_targets) if self.epoch < self.initial_epoch else alignment_metrics[
            #     pos_inds]
            ...
            loss_bbox = self.loss_bbox(
                ...
                # targets_decode=pos_bbox_targets_decode,
                avg_factor=1.0)
        else:
    ...
    #     assert not torch.isnan(centerness_targets).any()
