...
from mmcv.cnn import bias_init_with_prob, ConvModule, DepthwiseSeparableConvModule, normal_init
...
class TaskDecomposition(nn.Module):
    ...
    def forward(self, feat, avg_feat=None, max_feat=None):
        b, c, h, w = feat.shape
        if avg_feat is None:
            avg_feat = F.adaptive_avg_pool2d(feat, (1, 1))
        avg_weight = self.layer_attention(avg_feat)
        max_weight = self.layer_attention(max_feat)
...
class RotatedYOLOXHead_angle(YOLOXHead):
    ...
    def _init_layers(self):
        ...
        self.multi_level_inter_convs = nn.ModuleList()
        self.multi_level_cls_decomp = nn.ModuleList()
        self.multi_level_reg_decomp = nn.ModuleList()
        self.multi_level_ang_decomp = nn.ModuleList()
        
        for _ in self.strides:
            self.multi_level_inter_convs.append(self._build_inter_convs())
            conv_cls, conv_reg, conv_obj, conv_ang, cls_decomp, reg_decomp, ang_decomp = self._build_predictor()
            self.multi_level_conv_cls.append(conv_cls)
            self.multi_level_conv_reg.append(conv_reg)
            self.multi_level_conv_obj.append(conv_obj)
            self.multi_level_conv_ang.append(conv_ang)
            self.multi_level_cls_decomp.append(cls_decomp)
            self.multi_level_reg_decomp.append(reg_decomp)
            self.multi_level_ang_decomp.append(ang_decomp)
    ...
    def _build_predictor(self):
        ...
        cls_decomp = TaskDecomposition(self.feat_channels,
                                            self.inter_convs,
                                            self.inter_convs * 8,
                                            self.conv_cfg, self.norm_cfg)
        reg_decomp = TaskDecomposition(self.feat_channels,
                                            self.inter_convs,
                                            self.inter_convs * 8,
                                            self.conv_cfg, self.norm_cfg)
        ang_decomp = TaskDecomposition(self.feat_channels,
                                            self.inter_convs,
                                            self.inter_convs * 8,
                                            self.conv_cfg, self.norm_cfg)
        return conv_cls, conv_reg, conv_obj, conv_ang, cls_decomp, reg_decomp, ang_decomp

    def _build_inter_convs(self):
        inter_conv = []
        for i in range(self.inter_convs):
        ...
        return  nn.Sequential(*inter_conv)

    def init_weights(self):
        super(RotatedYOLOXHead_angle, self).init_weights()
        # Use prior in model initialization to improve stability
        bias_init = bias_init_with_prob(0.01)
        for conv_ang in self.multi_level_conv_ang:
            conv_ang.bias.data.fill_(bias_init)
        for cls_decomp in self.multi_level_cls_decomp:
            cls_decomp.init_weights()
        for reg_decomp in self.multi_level_reg_decomp:
            reg_decomp.init_weights()
        for ang_decomp in self.multi_level_ang_decomp:
            ang_decomp.init_weights()
        for inter_conv in self.multi_level_inter_convs:
            for m in inter_conv:
                normal_init(m.conv, std=0.01)

    def forward_single(self, x, conv_cls, conv_reg,
                       conv_obj, conv_ang, inter_conv, cls_decomp, reg_decomp, ang_decomp):
        """Forward feature of a single scale level."""

        inter_feats = []
        for inter in inter_conv:
            x = inter(x)
            inter_feats.append(x)
        feat = torch.cat(inter_feats, 1)
    
        avg_feat = F.adaptive_avg_pool2d(feat, (1, 1))
        max_feat = F.adaptive_max_pool2d(feat, (1, 1))
        
        cls_feat = cls_decomp(feat, avg_feat, max_feat)
        reg_feat = reg_decomp(feat, avg_feat, max_feat)
        ang_feat = ang_decomp(feat, avg_feat, max_feat)

        cls_score = conv_cls(cls_feat)
        bbox_pred = conv_reg(reg_feat)
        angle_pred = conv_ang(ang_feat)
    ...
    def forward(self, feats):
        ...
        return multi_apply(
            self.forward_single, feats, self.multi_level_conv_cls,
            self.multi_level_conv_reg, self.multi_level_conv_obj,
            self.multi_level_conv_ang, self.multi_level_inter_convs,
            self.multi_level_cls_decomp, self.multi_level_reg_decomp,
            self.multi_level_ang_decomp)
    ...
    def _get_target_single(self, cls_preds, objectness, priors, decoded_bboxes,
        ...
        return super(RotatedYOLOXHead_angle,
