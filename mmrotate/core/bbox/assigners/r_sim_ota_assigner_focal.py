...
class RSimOTAAssigner_focal(SimOTAAssigner):

    def __init__(self, angle_version='le90', **kwargs):
        super(RSimOTAAssigner_focal, self).__init__(**kwargs)
    ...
    def _assign(self,
        ...
        focal_weight = (self.alpha * gt_onehot_label + (1 - self.alpha) *
                    (1 - gt_onehot_label)) * pt.pow(self.gamma)
        ...
        cls_cost = (
            (F.binary_cross_entropy(
                ...
                gt_onehot_label,
                reduction='none',
            ) * focal_weight).sum(-1).to(dtype=valid_pred_scores.dtype))
