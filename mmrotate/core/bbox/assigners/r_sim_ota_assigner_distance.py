...
class RSimOTAAssigner_distance(SimOTAAssigner):

    def __init__(self, angle_version='le90', **kwargs):
        super(RSimOTAAssigner_distance, self).__init__(**kwargs)
    ...
    def get_in_gt_and_in_center_info(self, priors, gt_bboxes):
        ...
        # 计算中心点到各个特征点的距离 
        distance = torch.sqrt((repeated_x - gt_cxs) ** 2 + (repeated_y - gt_cys) ** 2)

        # 将小于center_radius * repeated_stride_x 的为is_in_cts
        is_in_cts = distance < self.center_radius * repeated_stride_x
        ...
        # is_in_cts = ct_deltas.min(dim=1).values > 0 # [num_priors ,1]
        # is_in_cts_all = is_in_cts.sum(dim=1) > 0  # [num_priors]
...
if __name__ == '__main__':
    ...
    ota = RSimOTAAssigner_distance()
    is_in_gts_or_centers, is_in_boxes_and_centers = ota.get_in_gt_and_in_center_info(priors, gt_bboxes)
    print(is_in_gts_or_centers) # 是否在 gts内或者 centers
    print(is_in_gts_or_centers.shape)
    print(is_in_boxes_and_centers) # 在 gts内 并且在 cetners 
    print(is_in_boxes_and_centers.shape)