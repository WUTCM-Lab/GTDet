...
class RSimOTAAssigner_gau_power(SimOTAAssigner):

    def __init__(self,
                 angle_version='le90',
                 candidate_topk=10,
                 **kwargs):
        super(RSimOTAAssigner_gau_power, self).__init__(**kwargs)
    ...
    def _assign(self,
        ...
        assigned_gt_inds = decoded_bboxes.new_full((num_bboxes, ),
                                                   0,
                                                   dtype=torch.long)
        valid_mask, gau_dis= self.get_in_gt_and_in_center_info(
            priors, gt_bboxes)
        ...
        gau_cost = torch.pow(10, (0.4 - gau_dis) * 10)
        # gau_cost  = torch.exp((1 - gau_dis) * 10)
        
        cost_matrix = soft_cls_cost * self.cls_weight + iou_cost * self.iou_weight + gau_cost
    ...
    def get_in_gt_and_in_center_info(self, priors, gt_bboxes):
        ...
        # cal gau_dis
        gau_dis = self.get_gau_dis(gt_bboxes, priors_cts[is_in_gts_all])
...
if __name__ == '__main__':
    ...
    eps = 1e-7
    assigner = RSimOTAAssigner_gau_exp()
    ...
    x = np.linspace(10, 15, 6, endpoint=True)

    gau_dis = assigner.get_gau_dis(gt, point)
    ...
    # plt.plot(x, gau_div, linestyle="solid", marker='v', c='red', label='gau_log')
    ...
    # plt.show()
    plt.savefig("/home/server4/mmrotate/figure/gau_dis_15.jpg")